...
- `examples/{node,bun,cloudflare,alchemy-cloudflare}/`: byte-identical one-page application starters; nonminimal features belong in Alchemy-based examples such as `basic -> alchemy`. Rich Node/Bun host regression apps are test-owned fixtures under `tests/e2e-server/fixtures/`.
