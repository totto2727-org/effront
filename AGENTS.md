...
- `examples/minimal/`: byte-identical one-page application starters for Node, Bun, standalone Cloudflare, and Alchemy-managed Cloudflare; richer integration showcases remain in `examples/`. `basic -> alchemy` is an alias excluded from workspace discovery.
