...
The suite owns ports 4451, 4452, 4453 and 4454 and starts production Node/Bun samples plus Node-hosted Vite development and preview.
Preview uses the Node sample output built by the first web server; preserve that startup ordering.
Do not run another build against those sample outputs while this suite is active.

## Architecture

- Exercise the committed application entries and actual standard Effect HTTP listeners, not a copied handler or test-only host.
