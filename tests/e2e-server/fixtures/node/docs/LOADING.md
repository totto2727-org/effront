...
Node 回帰テスト fixture の `/loading` で、ページ移動・コンポーネントの更新・ブラウザーでの取得を待つ間、画面のどこが表示されたままになるかを比較します。
...
```sh
vp install
cd tests/e2e-server/fixtures/node
vp dev --host 127.0.0.1 --port 4455 --strictPort
```

JavaScript を有効にしたブラウザーで `http://127.0.0.1:4455/loading` を開きます。
本番の Node HTTP サーバーと比較する場合は dev を停止し、`tests/e2e-server/fixtures/node` で次を実行します。
...
- [`query.e2e.ts`](../../../query.e2e.ts): SSR とブラウザーの開始境界、初回取得とキー変更、キャッシュ再取得、失敗後の再試行。
- [`loading.e2e.ts`](../../../loading.e2e.ts): 実ホストでの fallback、段階的な表示、状態保持、サーバーの待機区間。
...
Loading ページは Node fixture にのみあるため、そのケースは Bun プロジェクトでは実行しません。
Playwright 設定がサーバーを起動・停止します。
ポート 4451〜4454 を空け、実行中に同じ fixture を別途 build しないでください。
