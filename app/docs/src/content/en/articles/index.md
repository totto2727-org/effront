Effront renders React pages with Effect and handles browser actions through Server Functions.

## Get a page on screen {#boundaries}

[Getting started](./guide/getting-started.md) creates a homepage with a layout and a route.
Choose a [host setup](./platforms.md) to run it.

## Add data and user actions {#overview}

- [Routing](./guide/routes.md): map URLs to pages, read route parameters, and share layouts.
- [Service injection](./guide/effect.md): provide application services and use them while rendering a page.
- [Server Functions](./guide/server-functions.md): validate browser input and run server-side handlers.

## Choose your next guide {#next}

- Render articles with [Markdown](./guide/markdown.md) or add Tailwind CSS with [Styling](./guide/styling.md).
- Check service lifetimes and screen updates in [Runtime contracts](./advanced.md).
- Test application logic and browser behavior with [Testing applications](./best-practices/testing.md).
- Look up options and types in the [API reference](./api-reference.md).
- Follow the implementation in the [architecture walkthrough](/en/architecture/implementation/overview).
