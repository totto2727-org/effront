...
- Public packages are `@effront/core`, `@effront/vite`, `@effront/cloudflare`, `@effront/markdown`, and `@effront/tailwind`.
...
For the user-requested Alchemy prototype, keep existing `0.1.1` versions and keep `@effront/alchemy` private.
Do not publish the prototype or add it to release filters without a separate release decision.
